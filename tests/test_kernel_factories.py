...
from botorch.models.kernels.positive_index import (
    PositiveIndexKernel as GPyTorchPositiveIndexKernel,
)
...
def _matern_kernel_factory(searchspace, objective, measurements):
    """A callable kernel factory returning a plain BayBE Matern kernel.

    The kernel sets no parameter names. When combined with a transfer learning
    override, it is called on the reduced (task-free) search space to build the base
    kernel.
    """
...
def test_resolve_kernel_dispatch_success(
    ...
    assert isinstance(task_kernel, expected_task_kernel_cls)
