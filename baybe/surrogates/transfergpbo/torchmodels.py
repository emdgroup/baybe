...
class MHGPModel(Model):
    ...
    def _extract_task_data(self, X: Tensor, Y: Tensor, task_feature: int, target_task: int) -> Tuple[List[Tuple[Tensor, Tensor]], Tuple[Tensor, Tensor]]:
        ...
        if task_feature == -1:
            ...
            X_features = torch.cat([X[:, :task_feature], X[:, task_feature+1:]], dim=1)
    ...
    def meta_fit(self, X: Tensor, Y: Tensor, task_feature: int = -1, target_task: int = 2) -> None:
    ...
    def fit(self, X: Tensor, Y: Tensor, task_feature: int = -1, target_task: int = 2) -> None:
        ...
        """
        # Extract target data
        _, (X_target, Y_target) = self._extract_task_data(X, Y, task_feature, target_task)
    ...
    def posterior(self, X: Tensor, **kwargs) -> GPyTorchPosterior:
        ...
        if not self._fitted:
            raise RuntimeError(
                "Model must be fitted first. Call meta_fit() and fit()."
            )
    ...
    def _predict_from_stack(self, X: Tensor, up_to_task_id: int) -> Tuple[Tensor, Tensor]:
        ...
        Args:
            ...
            up_to_task_id: Task ID to predict up to (inclusive).
                        If equals len(self.source_gps), uses full stack including target GP.
