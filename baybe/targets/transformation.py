...
from attrs.validators import deep_iterable, gt, instance_of, is_callable, min_len
...
from baybe.utils.validation import finite_float, non_nan_float
...
class ClampingTransformation(MonotonicTransformation):
    ...
    min: float = field(default=float("-inf"), converter=float, validator=non_nan_float)
    """The lower cutoff value."""

    max: float = field(default=float("inf"), converter=float, validator=non_nan_float)
...
class PowerTransformation(Transformation):
    ...
    exponent: float = field(converter=float, validator=finite_float)
