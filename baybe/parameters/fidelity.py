...
import numpy as np
import pandas as pd
from attrs import Converter, define, field, Factory
from attrs.validators import and_, deep_iterable, ge, in_, instance_of, le, min_len, or_
import cattrs
...
from baybe.parameters.validation import validate_contains_one, validate_is_finite, validate_unique_values
...
class CategoricalFidelityParameter(DiscreteParameter):
    ...
    # object variables
    # Encoding is not used for CategoricalFidelityParameter. 
    ...
    _zeta: tuple[float, ...] | float | None = field(
        ...
        converter=lambda x: (
            None if x is None
            else cattrs.structure(x, float) if isinstance(x, (float, int))
            else cattrs.structure(x, tuple[float, ...])
        ),
        default=None,
    )
    """The maximum discrepancy from target fidelity at any design choice.
    A float Z is interpreted as the tuple ``(Z, 2*Z, ..., M*Z)``.
    Used in the MFUCB acquisition function."""
    ...
    def _validate_zeta( # noqa: DOC101, DOC103
        self, _: Any, value: tuple[float, ...] | float | None
    ) -> None:
        """Validate instance attribute ``zeta``.

        This attribute may be empty; a positive float; or a tuple of positive floats,
        one for each fidelity value.
    ...
    def _validate_target_fidelity( # noqa: DOC101, DOC103
        ...
        if not target_value in self._values:
            raise ValueError(
    ...
    def values(self) -> tuple:
        ...
        return [self._values[f] for f in sorted_fidelities]
    ...
    def costs(self) -> tuple:
        ...
        return [self._costs[f] for f in sorted_fidelities]
    ...
    def zeta(self) -> tuple:
        ...
        return [zeta_tup[f] for f in sorted_fidelities]
    ...
    def target_fidelity_comp(self) -> int:
        ...
        return self.comp_df.loc[self.target_fidelity, self.name]
